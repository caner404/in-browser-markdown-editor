<template>
  <div class="darkmodeContainer">
    <svg width="17" height="16" xmlns="http://www.w3.org/2000/svg" :class="{ 'theme-active': store.showDarkMode() }">
      <path
    ...
    <div class="themeToggle" @click="store.toggleDarkMode()">
      <button class="themeToggle-btn" :class="{ 'btn-active': !store.showDarkMode() }"></button>
    </div>
    <svg width="18" height="18" xmlns="http://www.w3.org/2000/svg" :class="{ 'theme-active': !store.showDarkMode() }">
      <path
...
.themeToggle-btn {
  border: none;
  background-color: var(--clr-document-name-white);
  border-radius: 50%;
  width: 1.5rem;
  height: 1.5rem;
...
.themeToggle-btn.btn-active {
  transform: translateX(2.5rem);
}
svg path {
  fill: var(--clr--600);
}
svg.theme-active path {
  fill: var(--clr-document-name-white);
}
</style>
